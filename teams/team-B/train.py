...
def plot_training_history(log_dict, save_path="./outputs/training_curves.png"):
    ...
    print("\n=== Training Summary ===")
    print(f"Best Training Loss: {df['train_loss'].min():.6f} (Epoch {df['train_loss'].idxmin()})")
    if has_val_metrics:
        print(f"Best Validation Loss: {df['val_loss'].min():.6f} (Epoch {df['val_loss'].idxmin()})")
    print(f"Best Training PSNR: {df['train_psnr'].max():.2f} dB (Epoch {df['train_psnr'].idxmax()})")
    if has_val_metrics:
