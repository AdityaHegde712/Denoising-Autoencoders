...
from utils import *
from architecture import Encoder, Decoder, Autoencoder
...
class Hybrid_L1_Charbonnier(nn.Module):
    def __init__(self, alpha: float = 0.5, charbonnier_eps: float = 1e-3, reduction: str = 'mean'):
        super().__init__()
        self.alpha = alpha
        self.l1 = nn.L1Loss(reduction=reduction)
        self.charbonnier = Charbonnier(eps=charbonnier_eps)
    
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        l1_loss = self.l1(pred, target)
        charbonnier_loss = self.charbonnier(pred, target)
        return self.alpha * l1_loss + (1 - self.alpha) * charbonnier_loss
...
class ConvolutionalAutoencoder():
    ...
    def _train_one_epoch(
        ...
        for step, (noisy, clean, noise) in pbar:
            noisy = noisy.to(self.cfg.device, non_blocking=True)
            noise = noise.to(self.cfg.device, non_blocking=True)
            ...
            with torch.no_grad():
                ...
                batch_mse = F.mse_loss(denoised, clean, reduction='none').view(bs, -1).mean(dim=1)
                batch_psnr = psnr_from_mse(batch_mse).mean().item()
                
                # batch_ssim = ssim(recon, noise, data_range=1.0, size_average=True).item()
    ...
    def test(self, checkpoint_path="./outputs/best.pt"):
        ...
        with torch.no_grad():
            for batch_idx, (noisy, clean, noise) in enumerate(self.test_loader):
